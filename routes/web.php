<?php
/*
|--------------------------------------------------------------------------
| Application Routes
|--------------------------------------------------------------------------
|
| Here is where you can register all of the routes for an application.
| It's a breeze. Simply tell Laravel the URIs it should respond to
| and give it the controller to call when that URI is requested.
|
*/

////cron route
//Route::get('cron', 'CronController@index');
Route::get('/clear-cache', function () {
    Artisan::call('cache:clear');
    Artisan::call('view:clear');
    Artisan::call('config:clear');
    return redirect('/');

});

//
//Route::get('update',
//    function () {
//        \Illuminate\Support\Facades\Artisan::call('migrate');
//        \Laracasts\Flash\Flash::success("Successfully Updated");
//        return redirect('/');
//    });
Route::get('migrate_seed',
    function () {
        \Illuminate\Support\Facades\Artisan::call('module:migrate');
        \Illuminate\Support\Facades\Artisan::call('module:seed');
        \Laracasts\Flash\Flash::success("Successfully Installed");
        return redirect('/');
    });
Route::get('migrate_c',
    function () {
        \Illuminate\Support\Facades\Artisan::call('module:migrate',['--force'=>true]);
        \Laracasts\Flash\Flash::success("Successfully migrated");
        return redirect('/dashboard');
    });
Route::get('seed_provisional',
    function () {
        \Illuminate\Support\Facades\Artisan::call('module:seed',['--force'=>true,'--class'=>'LoanProvisioningTableSeeder']);
        \Laracasts\Flash\Flash::success("Successfully migrated");
        return redirect('/dashboard');
    });
//Route::group(['prefix' => 'update'], function () {
//    Route::get('download', 'UpdateController@download');
//    Route::get('install', 'UpdateController@install');
//    Route::get('clean', 'UpdateController@clean');
//    Route::get('finish', 'UpdateController@finish');
//})
/**Route::get('/link', function () {
    $target = '/home/secapita/lendy.secapital.co.ke/storage/app/public';
    $shortcut = '/home/secapita/lendy.secapital.co.ke/public/storage';

    // Check if the symbolic link already exists
    if (!file_exists($shortcut)) {
        // Attempt to create the symbolic link
        if (symlink($target, $shortcut)) {
            return "Symbolic link created successfully.";
        } else {
            return "Failed to create symbolic link. Check server restrictions.";
        }
    } else {
        return "Symbolic link already exists.";
    }
}); **/
